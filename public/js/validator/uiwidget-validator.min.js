    ...
    a.extend(a.validator, {
        defaults: {
            ...
            onkeyup: function(a, b) {
                if (b.which === 9 && this.elementValue(a) === "") return; (a.name in this.submitted || a === this.lastActive) && this.element(a)
            },
        ...
        prototype: {
            ...
            formatAndAdd: function(b, c) {
                ...
                e = /\$?\{(\d+)\}/g;
                typeof d == "function" ? d = d.call(this, c.parameters, b) : e.test(d) && (d = a.validator.format(d.replace(e, "{$1}"), c.parameters)),
                this.errorList.push({
                    message: d,
                    element: b
                }),
            ...
            showLabel: function(b, c) {
                ...
                !c && this.settings.success && (d.text(""), typeof this.settings.success == "string" ? d.addClass(this.settings.success) : this.settings.success(d, b)),
            ...
            stopRequest: function(b, c) {
                ...
                c && this.pendingRequest === 0 && this.formSubmitted && this.form() ? (a(this.currentForm).submit(), this.formSubmitted = !1) : !c && this.pendingRequest === 0 && this.formSubmitted && (a(this.currentForm).triggerHandler("invalid-form", [this]), this.formSubmitted = !1)
            },
        ...
        attributeRules: function(b) {
            ...
            for (var e in a.validator.methods) {
                var f;
                e === "required" ? (f = d.get(0).getAttribute(e), f === "" && (f = !0), f = !!f) : f = d.attr(e),
...
function(a) { ! jQuery.event.special.focusin && !jQuery.event.special.focusout && document.addEventListener && a.each({
        focus: "focusin",
        blur: "focusout"
    },
    function(b, c) {
    ...
    $.extend($.validator.defaults, {
        unhighlight: function(A, _, B) {
            var C = $(A);
            if (C.is("select") && C.parent().is("span")) C.parent().replaceWith(C);
            else if (C.is(":checkbox, :radio")) $([]).add(this.currentForm.elements).filter("[name='" + C.attr("name") + "']").removeClass(_).addClass(B).attr("title", "");
            else C.removeClass(_).addClass(B).attr("title", "")
        },
        highlight: function(C, A, B) {
            var _ = $(C.element);
            if (_.is("select")) {
                if (_.parent().is("span")) _.parent().addClass(A).removeClass(B).attr("title", C.message);
                else _.wrap('<span class="' + A + '" title="' + C.message + '"/>')
            } else _.addClass(A).removeClass(B).attr("title", C.message)
        },
        showErrors: function() {
            for (var A = 0; this.errorList[A]; A++) {
                var $ = this.errorList[A];
                this.settings.highlight && this.settings.highlight.call(this, $, this.settings.errorClass, this.settings.validClass)
            }
            if (this.errorList.length) this.toShow = this.toShow.add(this.containers);
            if (this.settings.success) for (A = 0; this.successList[A]; A++) this.showLabel(this.successList[A]);
            if (this.settings.unhighlight) for (var A = 0,
            _ = this.validElements(); _[A]; A++) this.settings.unhighlight.call(this, _[A], this.settings.errorClass, this.settings.validClass);
            this.toHide = this.toHide.not(this.toShow);
            this.hideErrors();
            this.addWrapper(this.toShow).show()
        }
    ...
    $.extend($.validator.prototype, {
        elements: function() {
            var _ = this,
            A = {};
            return $([]).add(this.currentForm).find(":input").not(":submit, :reset, :image, [disabled]").not(this.settings.ignore).filter(function() { ! this.name && _.settings.debug && window.console && console.error("%o has no name assigned", this);
                if ((this.name in A) && $(this).is(":checkbox, :radio") || !_.objectLength($(this).rules())) return false;
                A[this.name] = true;
                return true
            })
        }
