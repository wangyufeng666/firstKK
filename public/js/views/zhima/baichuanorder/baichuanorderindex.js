...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/zhima/baichuanorder/pagelist'
...
function orderInfo(orderNo){
	layer.open({
		...
		title:'订单详情',
		shadeClose:false,
		content:"/zhima/baichuanorder/orderinfo?orderNo="+orderNo,
		area:['100%','100%'],
...
function stopOrder(orderNo){
	layer.open({
		...
		title:'订单超时终止',
		shadeClose:false,
		content:'/zhima/baichuanorder/tostoporder?overtimeFlag=Y&orderNo='+orderNo,
		area:['500px','350px'],
		close:function(index){
			layer.close(index);
		}
...
function toPay(orderNo){
	layer.open({
		...
		title:'订单支付',
		shadeClose:false,
		content:"/zhima/baichuanorder/topay?orderNo="+orderNo,
		area:['100%','100%'],
		close:function(index){
			layer.close(index);
		}
...
function dissentInfo(dissent, mobile, orderNo, orderType){
	var index = layer.confirm(dissent,{title:'客户异议', btn: ['发送未接通短信','关闭']},
			function(){
				if(confirm('是否确认发送短信？')){
					var params = {mobile:mobile,sendFlag:1,orderNo:orderNo,sourceCode:orderType};
					$.post('/zhima/baichuanorder/sendsms', params, function(data){
						if(data == 'Y'){
							layer.alert('短信发送成功');
						}else{
...
function downloadZhiMaExport(){
	...
	window.location.href = '/zhima/baichuanorder/zhimaexport?'+param;
	...
	if(sr.length>0){
		for(var i = 0 ; i < sr.length; i++){
			...
			$.ajax({
				type: 'POST',
				async:false,
				url: '/zhimaapp/baichuanorder/stoporder',
				data: params,
