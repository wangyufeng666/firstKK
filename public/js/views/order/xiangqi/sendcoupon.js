...
$('#btnCoupon').click(function(){
	...
	var params = {orderNo:orderNo, couponValue:couponValue, couponName:couponName, batchCode:batchCode, typeCode:typeCode};
	if(confirm('是否确认发放面额为'+couponValue+'元的'+couponName+'？')){
		if(couponValue > 0){
			$.post('/order/xiangqi/sendcoupon', params, function(data){
				if(data == 'Y'){
					successBox("发券成功");
...
$('#btnCash').click(function(){
	...
	if(cashValue > 0){
		var src = '/order/xiangqi/cashpaypage?orderNo='+orderNo+'&cash='+cashValue;
