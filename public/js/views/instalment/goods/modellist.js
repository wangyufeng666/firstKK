...
function addModel(productId){
    var url = '/instalment/goods/addmodel?productId='+ productId;
    layer.open({
        type:2,
        title:'添加型号',
        shadeClose:false,
...
function editModel(modelId){
    var url = '/instalment/goods/editmodel?modelId='+ modelId;
    layer.open({
        type:2,
        title:'修改型号',
        shadeClose:false,
...
function statusControl(modelId, status,statusText){
    ...
    layer.confirm(msg,{btn:['确定','取消'],closeBtn:0},function(){
        $.post("/instalment/goods/updatemodelstatus",params,function (res) {
            layer.close(index);
            if(res.code === '1000'){
                layer.msg('状态更新成功');
