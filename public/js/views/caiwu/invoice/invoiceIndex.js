...
$().ready(function(){
    grid = $('#grid').grid({
        ...
        ,cm:[
            ...
            ,{header:"发票编码", dataIndex:'INVOICEID', width:'100px',sortable:false}
            ,{header:"发票名称", dataIndex:'INVOICENAME', width:'100px',sortable:false}
            ,{header:"申请时间", dataIndex:'CREATEDATE', width:'140px',sortable:false}
            ,{header:"开票时间", dataIndex:'INVOICEOPENDATE', width:'140px',sortable:false}
            ,{header:"发票代码", dataIndex:'FPDM', width:'100px',sortable:false}
            ,{header:"发票号码", dataIndex:'FPHM', width:'80px',sortable:false}
            ,{header:"发票类型", dataIndex:'KPLX', width:'80px',sortable:false,
                ...
                renderer:function(value, data, rowIndex, colIndex, metadata){
                    ...
                    if(data['STATUS'] == '2'){
                        if((data['FPDM'] === '' || data['FPDM'] === null) && (data['FPHM'] === '' || data['FPHM'] === null)){
                            returnText += ' | <a class="a_link" href="javascript:deleteThis(\''+value+'\')" style="color:red;">删除</a>';
                        }
                        if(data['LPDFURL'] === '' || data['LPDFURL'] === null || data['LPDFURL'] === undefined) {
...
$('#exportlist').on('click',function(){
	...
	var param = '?invoiceCode='+invoiceCode+'&invoiceName='+invoiceName+'&sendeeName='+sendeeName;
	param += '&startDate='+startDate+'&endDate='+endDate+'&status='+status;
	window.location.href = '/caiwu/invoice/exprotinvoice'+param;
});