    ...
    ,cm : [
      ...
      ,{header: "操作", dataIndex: '', width:'150px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
          ...
          if(status == '1'){//待审核
        	  returnText+=' | <a href="javascript:void(0);" onclick="orderOperation(\''+orderNo+'\')" class="a_link">电话确认</a>';
        	  ...
        	  returnText+=' | <a href="javascript:void(0);" onclick="orderOperation(\''+orderNo+'\')" class="a_link">用户发货</a>';
          }else if(status == '7'){//待收货
        	  returnText+=' | <a href="javascript:void(0);" onclick="orderOperation(\''+orderNo+'\')" class="a_link">收货</a>';
          }
          
    	  //待检测
    	  if(status == '3' && orderType != '2' && orderType != '5' && orderType != '11'){
...
function sendCoupon(orderNo){
	if(confirm('是否确认给用户支付Thinkworld黑点？')){
		$.post('/thinkpad/order/sendcoupon', {orderNo:orderNo}, function(data){
			if(data == 'Y'){
				if(confirm('黑点发放成功')){
				    doSearch();
				}
