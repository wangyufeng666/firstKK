...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header:"回收单号", dataIndex:'RECYORDERNO', width:'150px',sortable:false}
			,{header:"库存状态", dataIndex:'INVENTORYSTATUSNAME', width:'70px',sortable:false}
            ,{header:"负责人", dataIndex:'SPONSOR', width:'70px',sortable:false}
            ,{header:"销售时间", dataIndex:'SALEDATE', width:'80px',sortable:false}
            ,{header:"退货时间", dataIndex:'RETURNDATE', width:'80px',sortable:false}
            ,{header:"商品品类", dataIndex:'CATEGORYNAME', width:'70px',sortable:false}
            ,{header:"商品名称", dataIndex:'PRODUCTNAME', width:'70px',sortable:false}
            ,{header:"订单总价", dataIndex:'COST_PRICE', width:'70px',sortable:false}
            ,{header:"运费总价", dataIndex:'EXPRESS_PRICE', width:'70px',sortable:false}
            ,{header:"其他费用", dataIndex:'OTHER_PRICE', width:'70px',sortable:false}
            ,{header:"总成本", dataIndex:'COST_PRICE', width:'70px',sortable:false}
            ,{header:"销售价格", dataIndex:'SALE_PRICE', width:'70px',sortable:false}
            ,{header:"毛利率(%)", dataIndex:'PROFIT_RATE', width:'70px',sortable:false}
            ,{header:"订单状态", dataIndex:'STATUSNAME', width:'70px',sortable:false}
            ,{header:"操作", dataIndex:'PKID', width:'60px',sortable:false,
                renderer:function(value, data, rowIndex, colIndex, metadata){
                	return "<a class='a_link' href='javaScript:saleDetailInfo(\"" + value + "\")'>详情</a>";
                }
            }
		]
		,url:'/sale/dailysale/returndetailpagelist'
...
function saleDetailInfo(detailId){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/sale/dailysale/saledetailinfo?detailId='+detailId,
		area:['600px' , '400px'],
		close:function(index){
			layer.close(index);
		}
...
function saleDetailExport(){
	...
	var param = '?saleBillNo='+saleBillNo+'&startDate='+startDate+'&endDate='+endDate+'&saler='+saler;
	
	window.location.href = '/sale/dailysale/exportreturndetails'+param;
}