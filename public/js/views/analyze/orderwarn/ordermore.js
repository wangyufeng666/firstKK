...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
            ...
            ,{header: "重复订单数量", dataIndex: 'SUMS', width:'5%',sortable:false}
            ,{header: "手机号 / 昵称", dataIndex: '', width:'10%',sortable:false
				,renderer:function(value, data, rowIndex, colIndex, metadata){
					return data['MOBILE']+' / '+data['UNAME'];
				}
			}
            ,{header: "用户异常等级", dataIndex: 'LEVEL', width:'5%',sortable:false}
		...
		,url : '/analyze/orderwarn/ordermorepagelist'
...
var backUrl = '/analyze/orderwarn/ordermore';
...
$('#exportlist').on('click',function(){
	...
	var dayNo = $('#dayNo').val();
	var param = '?merType='+merType+'&partnerCode='+partnerCode+'&orderSource='+orderSource+'&merName='+merName;
	param += '&mobile='+mobile+'&orderSum='+orderSum+'&dayNo='+dayNo;
	window.location.href = '/analyze/orderwarn/exportordermore'+param;
});
