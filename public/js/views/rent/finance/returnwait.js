...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header: "创建时间", dataIndex: 'PUCODE', width:'15%',sortable:false
            ...
            ,{header: "追款金额", dataIndex: 'PRICE', width:'10%',sortable:false}
            ,{header: "状态", dataIndex: 'STATUSTXT', width:'10%',sortable:false}
            ,{header: "退货备注", dataIndex: 'REREMARKS', width:'10%',sortable:false}
            ,{header: "财务备注", dataIndex: 'REMARK', width:'10%',sortable:false}
            ,{header:"操作", dataIndex:'', width:'15%', sortable:false,
				renderer:function(value, data, rowIndex, colIndex, metadata){
					var pkid = data['PKID'];
					var puCode = data['PU_CODE'];
					var html = "<a class='a_link' href='javaScript:audit(\""+pkid+"\")'>收款</a>";
...
$('#exportlist').on('click',function(){
	...
	var param = '?reCode='+reCode+'&puCode='+puCode+'&partnerCode='+partnerCode+'&startDate='+startDate+'&endDate='+endDate+'&status=1';
...
function audit(pkid){
	var title = '采购退货收款操作';
	var url = '/rent/finance/returnaudit?pkid='+pkid;
	var x = '500px';
	var y = '450px';
...
//终止
function stop(pkid){
	layer.confirm('确定终止当前收款记录？', {
	  btn: ['确定','取消'] //按钮
	}, function(){
		$.post('/rent/finance/returnstop',{pkid:pkid,status:'3'},function(data){
			var msg = '';
			if(data == 'Y'){
				msg = '操作成功';
			}else{
