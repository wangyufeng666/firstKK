...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header: "来源", dataIndex: 'SOURCENAME', width:'100px',sortable:false}
			...
			,{header: "重量", dataIndex: 'UNIT', width:'60px',sortable:false
				,renderer:function(value, data, rowIndex, colIndex, metadata){
					return data['UNIT']+' KG';
				}
			}
...
function sureOrder(orderNo){
	if(confirm('是否确认已经收到旧衣物，交易完成？')){
		$.post('/order/hborders/updateorderstatus',{orderNo:orderNo,orderStatus:66},function(data){
			if(data == 'Y'){
				alert('操作成功');
...
function stopOrder(orderNo){
	if(confirm('是否确认终止订单？')){
		$.post('/order/hborders/updateorderstatus',{orderNo:orderNo,orderStatus:99},function(data){
			if(data == 'Y'){
				alert('操作成功');
				grid.query(getParams());
			}else{
...
function editOrder(orderNo){
	layer.open({
		  ...
		  shade: 0.8,
		  area: ['500px', '300px'],
		  content: '/order/hborders/yuyue?orderNo='+orderNo
		}); 
}
...
function showRemark(orderNo){
	$.post('/order/hborders/jsonremark',{orderNo:orderNo}, function(data){
...
function exprotOrder(){
	...
	window.location.href = '/order/hborders/orderexport?'+param;
