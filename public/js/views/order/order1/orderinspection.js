...
function reinspectOrder(){
    var orderNo = $('#orderNo').val();
	window.location.href = '/order/order/orderinspection?orderNo='+orderNo+'&inspectFlag=Y'+'&backUrl='+backUrl;
}
...
function inspectConfirm(){
	...
	if((boxdetailcode == '' || boxdetailcode == undefined) && tradetype == '2'){
		errorBox('请选择货箱位子');
	}else{
		if(confirm('确认检测后，订单不能复检。是否已确认检测结果？')){
			$('#inspectConfirm').unbind('click');
			var orderNo = $('#orderNo').val();
			$.post('/order/order/confirminspection', {orderNo:orderNo}, function(data){
				if(data == 'Y'){
					alert('订单已确认检测');
