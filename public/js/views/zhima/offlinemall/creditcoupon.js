		...
		,cm:[
			...
			,{header:"操作", dataIndex:'', width:'10%', sortable:false,
				renderer:function(value, data, rowIndex, colIndex, metadata){
					...
					//待发券
					if((status == '8' || status == '4') && zhimaOrderType == '2'){
						returnText += ' | <a href="javascript:toPay(\''+orderNo+'\')" class="a_link">结算</a>';
					}else if(status == '20' && zhimaOrderType == '2'){
...
function dissentInfo(dissent){
	layerIndex = layer.open({
		type:1, shade:false, title:false, area:['650px', '200px'],
		content:'<div class="layer_notice">'+dissent+'</div>'
	});
}
