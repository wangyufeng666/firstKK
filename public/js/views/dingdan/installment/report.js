...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
		   ...
		   ,{header:"尾款", dataIndex:'', width:'60px', sortable:false,
		       renderer:function(value, data, rowIndex, colIndex, metadata){
		           return parseInt(data['NEW_PRODUCT_PRICE']) - parseInt(data['TOTAL_PRICE']);
		       }
		   }
		   ,{header:"手续费", dataIndex:'', width:'60px', sortable:false,
               renderer:function(value, data, rowIndex, colIndex, metadata){
                   return (parseInt(data['OLD_PRODUCT_PRICE']) - parseInt(data['NEW_PRODUCT_PRICE'])  + parseInt(data['TOTAL_PRICE']));
               }
           }
...
function exportReport(){
    ///dingdan/installment/exportreport
    ...
    window.location.href = "/dingdan/installment/exportreport?" + parmStr;
}
