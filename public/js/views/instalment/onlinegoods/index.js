...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header: "操作", dataIndex: '', width:'30%', sortable:false,
				renderer:function(value, data, rowIndex, colIndex, metadata){
                    ...
                    html += "<a class='a_link' href='javaScript:void(0)'  onclick='addLabel(\"" + data['ID'] + "\")'>添加标签</a>&nbsp;&nbsp;|&nbsp;&nbsp;";
		...
		,url:'/instalment/onlinegoods/goodslist'
...
function addGoods(){
    layer.open({
        ...
        shadeClose:false,
        shade:0.8,
        content:'/instalment/onlinegoods/addgoods',
        area:['550px','600px'],
...
function editProduct(productId){
	var url = '/instalment/onlinegoods/editgoods?productId=' + productId;
    layer.open({
        type:2,
        title:'修改商品',
        shadeClose:false,
...
function addLabel(productId){
    var url = '/instalment/onlinegoods/label?productId=' + productId;
    layer.open({
        type:2,
        title:'添加标签',
        shadeClose:false,
...
function modelList(productId){
	var url = '/instalment/onlinegoods/modellist?productId=' + productId;
...
function statusControl(productId, status,statusText){
    ...
    layer.confirm(msg,{btn:['确定','取消'],closeBtn:0},function(){
        $.post("/instalment/onlinegoods/statuscontrol",params,function (res) {
            layer.close(index);
            if(res.code === '1000'){
                layer.msg('状态更新成功');
                grid.reload();
