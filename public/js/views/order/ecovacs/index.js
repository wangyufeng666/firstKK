      ...
      ,{header: "操作", dataIndex: '', width:'130px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
    	  ...
    	  //待发券
    	      ...
    	      if(status =='5' || status == '66' || status == '98' ){
                  returnText += ' | <a href="javascript:void(0);" onclick="endCoupon(\''+orderNo+'\')" class="a_link">作废券</a>';
              }
          return returnText;
        }
      }
    ]
    ,url : '/order/ecovacs/pagelist'
...
function orderInfo(orderNo){
	window.location.href = "/order/ecovacs/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/ecovacs/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}

function endCoupon(orderNo){
    if(confirm('是否作废该订单券码？')){
        $.post('/order/ecovacs/endcoupon', {orderNo:orderNo}, function(data){
            if(data.success == 'Y'){
                alert("作废成功");
                window.location.href = window.location.href;
            }else{
                errorBox('作废失败"：'+data.data);
            }
        });
    }
}
