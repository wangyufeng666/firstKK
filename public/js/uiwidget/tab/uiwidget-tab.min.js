...
addContentTab:function(a){if(a.closeable!==false)a.closeable=true;if(tab.searchItem(a.id)){if(a.refresh===true){tab.loadTab(a);tab.searchItem(a.id).recordId=a.recordId}}else{this.addItem(a);tab.tabItems.length>1&&tab.setClosable(tab.tabItems[0].id,true)}this.activeTab(a)},getActiveContent:function(){return this.getTabContent(this.getActiveItem().id)},closeActiveTab:function(a){var b=tab.getActiveItem(),c=b.parent;this.closeTab(tab.getActiveItem().id);if(tab.tabItems.length<=0){c.closeable=false;this.addContentTab(c)}if(a===
true&&c!=undefined){tab.searchItem(c.id)?this.activeTab(c):this.addContentTab(c);c.reload&&window.setTimeout(function(){try{c.reload.apply(this,b.reloadArgs?b.reloadArgs:[])}catch(e){}},500)}}};d.fn.tab=function(a){return new d.uiwidget.Tab(this,a)};d.fn.resizeIframe=function(a){var b=this[0];try{b.height=(b.Document?b.Document.body.scrollHeight:b.contentDocument.body.offsetHeight)+a.offsetHeight}catch(c){}}})(jQuery);
