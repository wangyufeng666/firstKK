...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="tabletpcInfo(\''+data['SPID']+'\')">详情</a>';
        ...
        ,url : '/product/tabletpc/pagelist'
        ,baseParams:{merType:'K', merName:$('#merName').val(),pinpai:$('#pinpai').val()}
	});
});
 ...
 * @return
 */
function tabletpcInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/tabletpc/tabletpcinfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
 ...
 * @return
 */
function addTabletpc(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/tabletpc/addtabletpc?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/tabletpc/edittabletpc?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/tabletpc/deletetabletpc', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
