	...
	,cm : [
		...
		,{header: "操作", dataIndex: '', width:'130px', sortable:false,
			renderer : function(value, data, rowIndex, colIndex, metadata){
				...
				returnText+='| <a href="javascript:logview(\''+orderNo+'\',\''+applySource+'\')" class="a_link">订单日志</a>';
				return returnText;
			}
		}]
...
function outWarehouse(orderNo){
	layer.open({
		type:2,
		title:'商品出库',
		content:'/warehouse/warehousebox/outware?out_orders='+orderNo,
		area : ['400px', '300px'],
		offset : ['50px',''],
		close:function(index){
			layer.close(index);
...
function outWarehouses(){
	window.location.href = '/inventory/inventoryv2/outwarehousesindex';
}
 ...
 * 日志查看
 */
function logview(orderNo){
	layer.open({
		type:2,
		title:'日志查看',
		content:'/inventory/order/logview?orderNo='+orderNo,
		area:['620px','300px'],
		offset : ['50px',''],
		close:function(index){
			layer.close(index);
