		...
		,cm:[
            ...
            ,{header:"操作", dataIndex:'RETURNBILLNO', width:'110px',sortable:false,
                renderer:function(value, data, rowIndex, colIndex, metadata){
            		...
            		if(status == 1){
            			html += "<a class='a_link' href='javaScript:updateReturnStatus(\"" + value + "\",\"2\")'>提审</a> | ";
                	}else if(status == 2){
                		html += "<a class='a_link' href='javaScript:updateReturnStatus(\"" + value + "\",\"1\")'>撤销</a> | ";
                	}else if(status == 4){
                		html += "<a class='a_link' href='javaScript:updateReturnStatus(\"" + value + "\",\"2\")'>重新提审</a> | ";
                	}
                	if(status == 1 || status == 4 || status == 5){
                		html += "<a class='a_link' href='javaScript:delSaleReturnBill(\"" + value + "\")'>删除</a> | ";
                	}
                    return html;
                }
            }
...
$('#returnTemplateDownload').click(function(){
	window.location.href = '/template/xls/Finance_salebill_return_temp01.xls';
});


function returnDataImport(){
	layer.open({
		type:2,
		title:'销售退货数据导入',
		shadeClose:false,
		shade:0.8,
		content:'/recycle/salebill/returnimportpage?saleBillNo='+saleBillNo,
		area:['600px', '100%'],
		close:function(index){
			layer.close(index);
		}
...
function returnDataExport(){
	...
	var param = 'billStatus='+billStatus+'&saler='+saler+'&saleBillNo='+saleBillNo+'&returnBillNo='+returnBillNo;
