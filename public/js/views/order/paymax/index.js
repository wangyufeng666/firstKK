		...
		,cm:[
			...
			,{header: "操作", dataIndex: '', width:'130px', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
					...
					//待发券
					if(orderType == '215'|| orderType == '216'){
						if(status == '4'){
							returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\')" class="a_link">付款</a>';
						}else if(status == '20'){
...
function orderInfo(orderNo){
	window.location.href = "/order/paymax/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/paymax/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function downloadExport(){
	...
	window.location.href = '/order/paymax/paymaxexport?'+param;
