...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/idlefishpai/order/identifylist'
...
function quotePage(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/idlefishpai/order/quotepage?orderNo='+orderNo,
		area:['100%','100%'],
...
function confirmQuote(orderNo){
	if(confirm('是否确认保价？')){
	    $.post('/idlefishpai/order/confirmquote', {orderNo:orderNo}, function(data){
	        if(data == 'Y'){
	            grid.reload();
	        }else{
	          alert(data);
...
function orderInfo(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/idlefishpai/order/taborderinfo?orderNo='+orderNo,
		area:['100%','100%'],
		close:function(index){
			layer.close(index);
		}
