...
function withoutInfo(orderNo){
	layer.open({
		type:2,
		title:'代扣详情',
		content:"/idlefish/crediturge/withoutinfo?orderNo="+orderNo,
		area:['600px', '400px'],
...
function urgeInfo(orderNo){
	layer.open({
		type:2,
		title:'催缴详情',
		content:"/idlefish/crediturge/urgeinfo?orderNo="+orderNo,
		area:['800px', '600px'],
		close:function(index){
			layer.close(index);
		}
...
function downloadUrgeOrder(){
	...
	window.location.href = '/idlefish/crediturge/export?'+params;
