...
function checkImgVerifyCode(){
	...
	if($.trim(imgCode) == ''){
		...
		$.ajax({
			type:'POST'
			,url:"/system/index/checkimgcode"
			,data:{imgCode:imgCode, mobile:phoneNum}
			,cache:false
			,async:false
			,timeout:30000
			,dataType:'json'
	...
	if(checkphone()){
		if(checkImgVerifyCode()){
			...
			$.ajax({
				...
				,success:function(data){
					if(data == 'Y'){
						window.location.href = "/system/index/newpassword?mobile="+mobile;
					}else{
						$("#btn_submit").bind("click", function(){submit();});
						showTips(data);
					}
