...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/product/suningcooperate/pagelist'
...
function delSpuMount(pkId){
	if(confirm('是否确认删除该商品的挂载？')){
		$.post('/product/suningcooperate/delsuningspumount', {pkId:pkId}, function(data){
			if(data == 'Y'){
				grid.reload();
			}else{
				alert('删除失败：'+data);
...
function spuReset(pkId){
	if(confirm('是否确认启用该商品？')){
		$.post('/product/suningcooperate/suningspureset', {pkId:pkId}, function(data){
			if(data == 'Y'){
				grid.reload();
			}else{
				alert('启用失败：'+data);
...
function merMatch(pkId, partnerMerName){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/product/suningcooperate/suningspumatch?pkId='+pkId,
		area:['640px', '400px'],
...
function doAdd(){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/product/cooperate/addcooperate?partnerCode=10000187',
		area:['800px', '500px'],
		close:function(index){
			layer.close(index);
		}
