$().ready(function(){
	...
	$('.card', $('#stepItems .stepItem .cards')).click(function(e){
		...
		$.get('/report/merquote/nbattr',{spid:$('#spId').val(),attrs:checkVal.join(',')},function(datas){
			...
			if(datas.types && datas.types != null && datas.types.length > 0){
...
function formSubmit(){
	...
	if(spId != '' && typeId != ''){
		layer.open({type:2});
		window.location.href = '/report/merquote/detail?spid='+spId+'&typeId='+typeId;
	}else{
