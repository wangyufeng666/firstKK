    ...
    ,cm : [
      ...
      ,{header: "操作", dataIndex: '', width:'130px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
    	  ...
    	  if(orderType == '62'){
    	      if(status == '8'){
			      returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\',\''+data['SETTLEPRICE']+'\',\''+data['EVENTNAME']+'\',\''+data['ACTIVECODE']+'\')" class="a_link">发券</a>';
              }else if(status == '20'){
        	      returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\',\''+data['SETTLEPRICE']+'\',\''+data['EVENTNAME']+'\',\''+data['ACTIVECODE']+'\')" class="a_link">确认发券</a>';
              }
    	  }
          return returnText;
        }
 ...
 * @return
 */
function sendCoupon(orderNo,couponValue,couponName, eventCode){
	$.post('/order/dell/couponcounts', {couponValue:couponValue, eventCode:eventCode}, function(data){
		if(data > 0){
			if(confirm('是否确认发放面额为'+couponValue+'元的'+couponName+'？剩( '+data+' 张)')){
				if(couponValue > 0){
					$.post('/order/dell/sendcoupon', {orderNo:orderNo}, function(data){
						if(data == 'Y'){
							alert("发券成功");
							doSearch();
						}else{
			...
			errorBox(couponValue+'元的'+couponName+'不足');
		}
	});
...
function downloadOrder(){
	...
	window.location.href = '/order/dell/orderexport?'+param;
