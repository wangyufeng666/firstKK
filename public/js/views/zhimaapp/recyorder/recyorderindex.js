...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/zhimaapp/recyorder/pagelist'
...
function orderInfo(orderNo){
	layer.open({
		type:2,
		title:'订单详情',
		content:'/zhimaapp/recyorder/orderinfo?orderNo='+orderNo+'&layer=Y',
		shadeClose:false,
		shade:0.8,
		area:['100%' , '100%'],
...
function stopOrder(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/zhimaapp/recyorder/tostoporder?overtimeFlag=Y&orderNo='+orderNo,
		area:['500px','350px'],
		close:function(index){
			layer.close(index);
		}
...
function toPay(orderNo){
	layer.open({
		type:2,
		title:'订单详情',
		content:'/zhimaapp/recyorder/topay?orderNo='+orderNo+'&layer=Y',
		shadeClose:false,
		shade:0.8,
		area:['100%' , '100%'],
		close:function(index){
	...
	var index = layer.confirm(dissent, {
		title:'客户异议',
		  btn: ['发送未接通短信','关闭'] //按钮
			...
			if(confirm('是否确认发送短信？')){
				$.post('/zhimaapp/recyorder/sendsms',{mobile:mobile,sendFlag : 1,orderNo:orderNo,sourceCode:orderType},function(data){
					if(data == 'Y'){
						layer.alert('短信发送成功');
					}else{
...
function downloadZhiMaExport(){
	...
	window.location.href = '/zhimaapp/recyorder/zhimaexport?'+param;
    ...
    if(sr.length>0){
        for(var i = 0 ; i < sr.length; i++){
            ...
            $.ajax({
                type: 'POST',
                async:false,
                url: '/zhimaapp/recyorder/stoporder',
                data: params,
