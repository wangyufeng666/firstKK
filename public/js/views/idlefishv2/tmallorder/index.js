...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header:"结算方式", dataIndex:'IDLE_PAY_TYPENAME', width:'80px',sortable:false}
		...
		,url:'/idlefishv2/tmallorder/pagelist'
...
function orderInfo(orderNo){
	layer.open({
		type:2,
		title:'订单详情',
		content:'/idlefishv2/tmallorder/orderinfo?orderNo='+orderNo+'&layer=Y',
		shadeClose:false,
		shade:0.8,
		area:['100%' , '100%'],
...
function stopOrder(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/idlefishv2/recyorder/tostoporder?overtimeFlag=Y&orderNo='+orderNo,
		area:['500px','350px'],
		close:function(index){
			layer.close(index);
		}
...
function toPay(orderNo){
	layer.open({
		type:2,
		title:'订单详情',
		content:'/idlefishv2/tmallorder/topay?orderNo='+orderNo+'&layer=Y',
		shadeClose:false,
		shade:0.8,
		area:['100%' , '100%'],
		close:function(index){
...
function getParams(){
	return {
		...
		merType:$('#merType').val(),
		idle_pay_type:$('#idle_pay_type').val(),
...
function downloadZhiMaExport(){
	...
	param += '&idle_pay_type=' + $('#idle_pay_type').val();
	param += '&zm_Order_Status=' + $('#zm_Order_Status').val();
	param += '&zm_Order_Type=' + $('#zm_Order_Type').val();
	window.location.href = '/idlefishv2/tmallorder/zhimaexport?'+param;
...
function downloadNumExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/tmallorder/numexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function downloadFailExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/recyorder/failexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function successEvaluationExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/tmallorder/successevaluationexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function cancelReasonExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/tmallorder/cancelreasonexport?'+param;
	return false; //截取返回false就不会保存网页了
}
