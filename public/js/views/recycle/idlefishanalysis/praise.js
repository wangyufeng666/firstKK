...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header:"评论时间", dataIndex:'CREATEDATE', width:'80px',sortable:false}
			...
			,{header:"评价", dataIndex:'RATE_GRADE', width:'80px',sortable:false
			...
			,{header:"评论内容", dataIndex:'RATE_CONTENT', width:'80px',sortable:false
				,renderer:function(value, data, rowIndex, colIndex, metadata){
					return '<span ondblclick="showRemark(\''+value+'\')">'+'<font color="blue">'+value+'</font></span>';
				}
			}
		...
		,url:'/recycle/idlefishanalysis/praisepagelist'
...
function download(){
	var param = getParams();
	var params = 'startDate='+param.startDate+'&endDate='+param.endDate+'&merType='+param.merType+'&brandCode='+param.brandCode+'&channel='+param.channel+'&orderSource='+param.orderSource+'&flag=2';
...
function showRemark(text){
	...
	layerIndex = layer.open({
		...
		content:'<div class="layer_notice">'+text+'</div>'
	});
}