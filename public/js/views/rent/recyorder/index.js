...
function orderInfo(orderNo){
	window.location.href = "/rent/recyorder/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function orderOperation(orderNo){
	$.layer({
		type:2,
		title:'订单状态修改',
		iframe:{src:'/rent/recyorder/operation?orderNo='+orderNo},
		area:['500' , '320'],
		offset:['50px',''],
...
function orderInspection(orderNo, merType){
	window.location.href = "/rent/recyorder/orderinspection?orderNo="+orderNo;
}
...
function jdOrderInspection(orderNo){
	window.location.href = "/rent/jdorder/orderinspection?orderNo="+orderNo;
}
...
function stopOrder(orderNo, sourceCode){
	var url = '/rent/recyorder/tostoporder?orderNo='+orderNo;
	var title = '订单终止';
