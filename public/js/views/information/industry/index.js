	...
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "标题", dataIndex: 'TITLE', width:'10%',sortable:false}
			,{header: "副标题", dataIndex: 'TITLEEXT', width:'10%',sortable:false}
           ,{header: "位置", dataIndex: 'IMGPOSITIONNAME', width:'9%',sortable:false}
				...
				renderer : function(value, data, rowIndex, colIndex, metadata){
           			var returnText = '';
						returnText += '<a class="a_link" href="javascript:void(0);" onclick="edit(\''+data['PKID']+'\')">编辑</a>';
					 	returnText += '&nbsp;|&nbsp;<a class="a_link" href="javascript:void(0);" onclick="importNews(\''+data['PKID']+'\')">导入PC</a>';
...
function addIndustry(){
	window.location.href='/information/industry/addnews?newsSource=1';
}

function edit(pkId) {
	window.location.href='/information/industry/updatenews?newsSource=1&pkId='+pkId;
}

/**
 * 是否讲新闻导入PC模块
 */
function importNews(pkId){
	if (confirm('是否将本新闻导入到PC端后台!')){
		var params = {};
		params.pkId = pkId;
		$.ajax({
			type:'POST'//请求方式
			,url:"/information/industry/importnews"  //请求路径
			...
			,success:function(data){
				if (data.code == 1000){
					layer.msg('导入成功');
					setTimeout("window.location.href='/information/industry/index';",2000);
				}else{
					layer.msg('导入失败');
