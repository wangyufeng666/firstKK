...
include('zmsdk/RSAUtil.php');
include('zmsdk/WebUtil.php');
class ZmopClient {
	//应用ID
	public $appId;
	//私钥文件路径，绝对路径或者相对路径
	...
	function __construct($gatewayUrl = '', $appId = '', $charset = 'UTF-8'){
	    $this -> gatewayUrl = $gatewayUrl;
		$this -> appId = $appId;
		$this -> privateKeyFilePath = dirname(__FILE__)."/rsa_private_key.pem";
		//芝麻公钥文件(注意：芝麻信用给的公钥只能复制，但是复制出来的格式是不对的，需要手工处理每行64，和生成的密钥格式一致，不然加密和解密将不能进行；)
        $this->  zhiMaPublicKeyFilePath = dirname(__FILE__)."/rsa_public_key.pem";
