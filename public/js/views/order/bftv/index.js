...
function orderInfo(orderNo){
	window.location.href = "/order/bftv/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo,couponValue,couponName){
	if(confirm('是否确认发放面额为'+couponValue+'元的'+couponName+'？')){
		if(couponValue > 0){
			$.post('/order/bftv/sendcoupon', {orderNo:orderNo}, function(data){
				if(data == 'Y'){
					successBox("发券成功");
					doSearch();
				}else{
