...
$().ready(function(){
	...
	grid = $('#grid').grid({
		...
		,url:'/recycle/brokeragestrategy/pagelist'
...
function strategyInfo(strategyCode){
	window.location.href = "/recycle/brokeragestrategy/strategyinfo?strategyCode="+strategyCode+"&backUri="+backUri;
}
...
function editstrategy(strategyCode){
	$.layer({
		type:2,
		title:'修改策略信息',
		iframe:{src:'/recycle/brokeragestrategy/editstrategyinfo?strategyCode='+strategyCode+'&backUri='+backUri},
		area:['750','550'],
		offset:['50px',''],
    ...
    if(confirm('是否将活动置为终止？')){
    	if(strstartDate>currentdate){
    		$.post('/recycle/brokeragestrategy/stopstrategytostatus', {strategyCode:strategyCode}, function(data){
    			if(data == 'Y'){
    				layer.alert("终止成功",1);
    				window.location.href ='/recycle/brokeragestrategy/index';
    			}else{
    				alert('终止失败,请重试。');
    			}
    		});
...
function addBrokerageStrategy(){
	window.location.href = '/recycle/brokeragestrategy/addbrokeragestrategy?backUri='+backUri;
}
