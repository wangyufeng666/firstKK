			...
			,{header: "操作", dataIndex: 'ORDERNO',width:'160px',sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
					...
					if(orderStatus == '1'){//待确认
						...
						returnText += ' | <a class="a_link" href="javascript:userTradeConfirm(\''+value+'\')">交易反馈</a>';
					}else if(orderStatus == '7'){
						returnText += ' | <a class="a_link" href="javascript:tradeConfirm(\''+value+'\')">用户确认</a>';
					}else if(orderStatus == '20'){//终止-待退回
                        returnText += ' | <a class="a_link" href="javascript:stopReturnConfirm(\''+value+'\')">退回确认</a>';
                    }else if(orderStatus == '21'){//终止-待发货
                        returnText += ' | <a class="a_link" href="javascript:stopReturnSendOut(\''+value+'\')">退回发货</a>';
                    }else if(orderStatus == '22'){//终止-待收货
                        returnText += ' | <a class="a_link" href="javascript:stopReturnReceived(\''+value+'\')">退回收货</a>';
                        ...
                        returnText += ' | <a class="a_link" href="javascript:tradeReturnSendOut(\''+value+'\')">交易发货</a>';
                    }else if(orderStatus == '32'){//交易-待收货
                        returnText += ' | <a class="a_link" href="javascript:tradeReturnReceived(\''+value+'\')">交易收货</a>';
                    }
					return returnText;
				}
			}
		]
		,url : '/identify/ifishorder/pagelist'
...
function orderCancel(orderNo){
	if(confirm('是否确定取消订单?')){
		$.post('/identify/ifishorder/ordercancel', {orderNo:orderNo}, function(data){
			if(data == 'Y'){
				grid.reload();
			}else{
				alert('订单取消失败：'+data);
 ...
 * @returns
 */
function stopReturnSendOut(orderNo){//终止待发货
    if(confirm('是否确定鉴定商品发货?')){
        $.post('/identify/ifishorder/stopreturnsendout', {orderNo:orderNo}, function(data){
            if(data == 'Y'){
                grid.reload();
            }
        });
 ...
 * @returns
 */
function stopReturnReceived(orderNo){//终止待收货
    if(confirm('是否确定商品退回用户已收货?')){
        $.post('/identify/ifishorder/stopreturnreceived', {orderNo:orderNo}, function(data){
            if(data == 'Y'){
                grid.reload();
            }
        });
 ...
 * @returns
 */
function tradeReturnSendOut(orderNo){//交易待发货
    if(confirm('是否确定交易发货?')){
        $.post('/identify/ifishorder/tradesendout', {orderNo:orderNo}, function(data){
            if(data == 'Y'){
                grid.reload();
            }
        });
 ...
 * @returns
 */
function tradeReturnReceived(orderNo){//交易待收货
    if(confirm('是否确定交易成功，用户已收货?')){
        $.post('/identify/ifishorder/tradereceived', {orderNo:orderNo}, function(data){
            if(data == 'Y'){
                grid.reload();
            }
        });
...
function orderReceived(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/identify/ifishorder/receivepage?orderNo='+orderNo,
		area:['500px','300px'],
...
function identifyReportInfo(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/identify/ifishorder/reportpage?orderNo='+orderNo,
		area:['640px','98%'],
		close:function(index){
			layer.close(index);
		}
...
function orderInfo(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/identify/ifishorder/taborderinfo?orderNo='+orderNo,
		area:['100%','100%'],
		close:function(index){
			layer.close(index);
		}
...
function orderIndentify(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/identify/ifishorder/identifypage?orderNo='+orderNo,
		area:['100%','100%'],
		close:function(index){
			layer.close(index);
		}
...
function pushOrderIndentify(orderNo){
	if(confirm('是否确认平台发布竞拍消息?')){
		$.post('/identify/ifishorder/pushidentifyreport', {orderNo:orderNo}, function(data){
			if(data == 'Y'){
				grid.reload();
			}
		});
...
//用户反馈
function userTradeConfirm(orderNo){
	layer.open({
		type:2,
		title:'商品鉴定-用户确认',
		shadeClose:false,
		shade:0.8,
		content:'/identify/ifishorder/sendbackpage?orderNo='+orderNo,
		area:['600px','500px'],
		close:function(index){
			layer.close(index);
		}
...
function tradeConfirm(orderNo){
	layer.open({
		...
		title:'退回待确认',
		shadeClose:true,
		content:'/identify/ifishorder/sendbackpage?orderNo='+orderNo+'&operType=2',
		area:['600px','450px'],
		close:function(index){
			layer.close(index);
		}
...
function tradeReturnConfirm(orderNo){
	layer.open({
		...
		title:'确认交易退回',
		shadeClose:true,
		content:'/identify/ifishorder/sendbackpage?orderNo='+orderNo+'&operType=2&returnFlag=TRADE',
		area:['600px','450px'],
		close:function(index){
			layer.close(index);
		}
...
function stopReturnConfirm(orderNo){
	layer.open({
		...
		title:'确认终止退回',
		shadeClose:true,
		content:'/identify/ifishorder/sendbackpage?orderNo='+orderNo+'&operType=1&returnFlag=STOP',
		area:['600px','450px'],
		close:function(index){
			layer.close(index);
		}
