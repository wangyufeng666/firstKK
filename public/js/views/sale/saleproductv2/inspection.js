		...
		cm:[
			...
			,{header:"操作", dataIndex:'', width:'200px', sortable:false,
				renderer:function(value, data, rowIndex, colIndex, metadata){
					...
					if(status == '70'){//退货寄回中状态-》才能确认收货
							returnText +=' | <a href="javascript:orderInspection(\''+batchCode+'\',\''+orderNo+'\', 70)" class="a_link">退货复检</a>';
					}
					if(status == '71'){//退货寄回中状态-》才能确认收货
						returnText +=' | <a href="javascript:printInspection(\''+batchCode+'\',\''+saleNo+'\',\''+orderNo+'\')" class="a_link">打印退货检测单</a>';
					}
					return returnText;
				}
			}
		]
		,url:'/sale/saleproductv2/inspectionlist'
 ...
 * @returns
 */
function printInspection(batchCode, saleNo,orderNo){
	layer.open({
		type:2,
		title:'打印退货检测单',
		shadeClose:false,
		shade:0.8,
		content:'/sale/saleproductv2/printinspect?batchCode='+batchCode+'&saleNo='+saleNo+'&orderNo='+orderNo,
		area:['90%','90%'],
...
function orderInspection(batchCode,orderNo) {
	layer.open({
		type:2,
		title:'退货单检测',
		shadeClose:false,
		shade:0.8,
		content:'/sale/saleproductv2/orderinspection?orderNo='+orderNo+'&batchCode='+batchCode,
		area:['90%','90%'],
		close:function(index){
			layer.close(index);
		},
