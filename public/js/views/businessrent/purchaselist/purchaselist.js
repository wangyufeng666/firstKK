	...
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           renderer:function(value, data, rowIndex, colIndex, metadata){
                ...
                var returnText ='<a href="javascript:void(0);" title="'+orderNo+'" onclick="sonPurchase(\''+orderNo+'\')" class="a_link">查看</a>';
                returnText +=' | <a href="javascript:void(0);" title="'+orderNo+'" onclick="exprotSonPurchase(\''+orderNo+'\')" class="a_link">下载报表</a>';
                return returnText;
              }
          }     
		]
...
function sonPurchase(orderNo){
      window.location.href = "/businessrent/order/sonorderpurchaselist?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function exprotPurchase(){
    ...
    window.location.href = '/businessrent/order/exprotpurchase?'+param;
    return false; //截取返回false就不会保存网页了
}
function exprotSonPurchase(orderNo){
    var param = '';
    param += '&orderNo=' + orderNo;
    param += '&isPurchase=' + '1';
    param += '&status=' + '3';
    window.location.href = '/businessrent/order/exprotsonpurchase?'+param;
    return false; //截取返回false就不会保存网页了
}
