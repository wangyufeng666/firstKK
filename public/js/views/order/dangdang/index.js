...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header: "当当用户编号", dataIndex: 'YHDUSERID', width:'87px',sortable:false}
			...
			,{header: "操作", dataIndex: '', width:'130px', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
					...
					//待发券
					if(orderType == '170'||orderType == '171'){
						if(status == '8'){
							returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\')" class="a_link">发券</a>';
						}else if(status == '20'){
							returnText += ' | <a href="javascript:void(0);" onclick="sendCouponOver(\''+orderNo+'\')" class="a_link">确认发券</a>';
						}
					}
					return returnText;
				}
...
function orderInfo(orderNo){
	window.location.href = "/order/dangdang/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/dangdang/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}
 ...
 * @return
 */
function sendCouponOver(orderNo){
	if(confirm('是否确认已支付？')){
		var params = {orderNo:orderNo};
		$.post('/order/dangdang/sendcouponover', params, function(data){
			if(data == 'Y'){
				successBox("发券已完成");
				window.location.href = window.location.href;
			}else{
				errorBox('确认发券失败：'+data);
			}
		});
	}
}
...
function downloadDdExport(){
	...
	window.location.href = '/order/dangdang/dangdangexport?'+param;
