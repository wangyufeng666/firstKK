...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/idlefishv2/totalrecyorder/pagelist'
...
function stopOrder(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/idlefishv2/totalrecyorder/tostoporder?overtimeFlag=Y&orderNo='+orderNo,
		area:['500px','350px'],
...
function toPay(orderNo){
	window.location.href = "/idlefishv2/totalrecyorder/topay?orderNo="+orderNo+"&backUrl="+backUrl;
}
/**
 * 异议
 * @param dissent
...
function downloadZhiMaExport(){
	...
	window.location.href = '/idlefishv2/totalrecyorder/zhimaexport?'+param;
...
function downloadNumExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/totalrecyorder/numexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function downloadFailExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/totalrecyorder/failexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function successEvaluationExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/totalrecyorder/successevaluationexport?'+param;
	return false; //截取返回false就不会保存网页了
}
...
function cancelReasonExport(){
	...
	param += '&startDate=' + $('#startDate').val();
	param += '&endDate=' + $('#endDate').val();
	window.location.href = '/idlefishv2/totalrecyorder/cancelreasonexport?'+param;
	return false; //截取返回false就不会保存网页了
}
    ...
    if(sr.length>0){
        for(var i = 0 ; i < sr.length; i++){
            ...
            $.ajax({
                type: 'POST',
                async:false,
                url: '/idlefishv2/totalrecyorder/stoporder',
                data: params,
