	...
	grid = $('#grid').grid({
		...
		,cm:[
            ...
            ,{header:"操作", dataIndex:'', width:'15%', sortable:false,
				renderer:function(value, data, rowIndex, colIndex, metadata){
					var reCode = data['RECODE'];
					var html = "<a class='a_link' href='javaScript:audityes(\""+reCode+"\")'>通过</a>";
					    html += "&nbsp;|&nbsp;<a class='a_link' href='javaScript:auditno(\""+reCode+"\")'>未通过</a>";
				    return html;
				}
			}
		]
...
$('#exportlist').on('click',function(){
	...
	var param = '?reCode='+reCode+'&puCode='+puCode+'&reName='+reName+'&puName='+puName+'&userName='+userName;
		param += '&startDate='+startDate+'&endDate='+endDate+'&status=2';
...
//审核通过
function audityes(reCode){
	if(reCode){
		layer.confirm('确认审核通过，等待财务追款？', {
		  btn: ['确定','取消'] //按钮
		}, function(){
			updateStatus(reCode,3);
		}, function(){
...
//审核未通过
function auditno(reCode){
	if(reCode){
		layer.confirm('确认审核未通过？', {
		  btn: ['确定','取消'] //按钮
		}, function(){
			updateStatus(reCode,5);
		}, function(){
...
//修改状态
function updateStatus(reCode,status){
	$.post('/rent/purchase/auditreturn',{reCode:reCode,status:status},function(data){
		var msg = '';
		if(data == 'Y'){
			msg = '操作成功';
		}else{
