...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="televisionInfo(\''+data['SPID']+'\')">详情</a>';
        ...
        ,url : '/product/television/pagelist'
        ,baseParams:{merType:'S', merName:$('#merName').val(),pinpai:$('#pinpai').val()}
	});
});
 ...
 * @return
 */
function televisionInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/television/televisioninfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
...
function addTV(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/television/addtelevision?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/television/edittelevision?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/television/deletetelevision', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
