...
$().ready(function(){
	grid = $('#grid').grid({
        ...
        ,url : '/product/experience/pagelist'
	});
});
...
function ExperienceInfo(Id){
	window.location.href = "/product/experience/experienceinfo?Id="+Id;
}
...
function addExperience(){
	window.location.href = "/product/experience/addexperience";
}
...
function approvalexperience(Id){
	...
	window.location.href="/product/experience/approvalexperience?Id="+Id;
}
...
function deleteThis(Id){
	if(confirm("是否确认删除当前数据？")){
		$.ajax({
			type: "POST",
			dataType:"json",
			url:'/product/experience/delete',
			data:{Id:Id},
			success: function(data){
				if(data=='1'){
					//alert("删除成功");
					window.location.href='/product/experience/index';
				}else{
					alert("删除失败");
				}
	        },
