...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
            ...
            ,{header: "所属品类", dataIndex: 'TYPENAME', width:'100px',sortable:false}
            ,{header: "自定义异常描述", dataIndex: 'ABNORMAL_DESC', width:'150px',sortable:false}
...
$('#exportlist').on('click',function(){
	...
	var param = '?mobile='+mobile+'&level='+level+'&sysname='+sysname+'&category='+category+'&merType='+merType+'&delflag='+delflag;
	window.location.href = '/yonghu/level/exportlist'+param;
});
...
function updatelevel(pkid){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/yonghu/level/updatelevel?pkid='+pkid,
		area:['450px' , '350px'],
		close:function(index){
			layer.close(index);
		}
...
function delwarn(pkid,status){
	...
	layer.confirm("确定"+txt+"该标识？", {
		  btn: ['确定','取消'] 
			...
			if(pkid !=''){
		        $.post("/yonghu/level/updatedelflag", {pkid:pkid,status:status}, function(data){
		            if(data == 'Y'){
		                layer.msg(txt+'成功');
