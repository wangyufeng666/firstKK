...
$('#btnCoupon').click(function(){
	...
	var params = {orderNo:orderNo, orderPrice:orderPrice};
	if(confirm('是否确认发放面额为'+orderPrice+'元的券码？')){

		if(orderPrice > 0 ){
			$.post('/bierengle/order/sendcoupon', params, function(data){
				if(data == 'Y'){
					successBox("发券成功");
...
$('#btnCash').click(function(){
	...
	if(cashValue > 0){
		var src = '/bierengle/order/cashpaypage?orderNo='+orderNo+'&cash='+cashValue;
