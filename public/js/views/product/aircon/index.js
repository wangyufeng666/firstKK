		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="airconInfo(\''+data['SPID']+'\')">详情</a>';
 ...
 * @return
 */
function airconInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/aircon/airconinfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
...
function addAircon(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/aircon/addaircon?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/aircon/editaircon?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/aircon/deleteaircon', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
