...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header:"预卖金额", dataIndex:'AUCTIONPRICE', width:'65px',sortable:false}
			,{header:"寄卖金额", dataIndex:'SETTLEPRICE', width:'65px',sortable:false}
...
function quotePage(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/zhuanzhuanv2/recyorder/quotepage?orderNo='+orderNo,
		area:['100%','100%'],
...
function confirmQuote(orderNo){
	if(confirm('是否确认保价？')){
	    $.post('/zhuanzhuanv2/recyorder/confirmquote', {orderNo:orderNo}, function(data){
	        if(data == 'Y'){
	            grid.reload();
	        }else{
	          alert(data);
