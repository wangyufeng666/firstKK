...
function sendCoupon(orderNo){
	//window.location.href = "/order/lemall/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
    if(confirm("是否确认发券？")){
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/feiniu/feiniu/sendcoupon',
            data:{orderNo:orderNo},
            success: function(data){
                if(data=='Y'){
                    window.location.href = "/feiniu/feiniu/index";
                }else{
                    alert("发券失败");
                }
            },
