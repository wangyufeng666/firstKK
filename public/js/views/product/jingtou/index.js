...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="jingtouInfo(\''+data['SPID']+'\')">详情</a>';
        ...
        ,url : '/product/jingtou/pagelist'
        ,baseParams:{merType:'B', merName:$('#merName').val(),pinpai:$('#pinpai').val()}
	});
});
 ...
 * @return
 */
function jingtouInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/jingtou/jingtouinfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
...
function addJingtou(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/jingtou/addjingtou?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/jingtou/editjingtou?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/jingtou/deletejingtou', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
