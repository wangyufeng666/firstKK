...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="washerInfo(\''+data['SPID']+'\')">详情</a>';
        ...
        ,url : '/product/washer/pagelist'
        ,baseParams:{merType:'R', merName:$('#merName').val(),pinpai:$('#pinpai').val()}
	});
});
 ...
 * @return
 */
function washerInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/washer/washerinfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
...
function addWasher(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/washer/addwasher?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/washer/editwasher?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/washer/deletewasher', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
