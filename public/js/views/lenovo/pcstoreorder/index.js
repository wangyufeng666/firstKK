...
$().ready(function(){
  grid = $('#grid').grid({
    ...
    ,cm : [
      ...
      ,{header: "所属品类", dataIndex: 'MERTYPENAME', width:'80px',sortable:false}
      ...
      ,{header: "店员信息", dataIndex: '', width:'130px',sortable:false
    	  ,renderer : function(value, data, rowIndex, colIndex, metadata){
    		  return data['USERNAME']+'('+data['PHONENUM']+')';
    	  }
      } 
      ...
      ,{header: "订单价格", dataIndex: '', width:'80px',sortable:false
          ,renderer:function(value, data, rowIndex, colIndex, metadata){
              var text = data['ORDERPRICE'];
              if(data['SETTLEPRICE']){
                  text += '(<span style="color:green">'+data['SETTLEPRICE']+'</span>)';
              }
              return text;
          }
      }
      ...
      ,{header: "操作", dataIndex: '', width:'120px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
    	  ...
    	  returnText += ' |<a href="javascript:void(0);" onclick="operatorOrder(\''+data['OPERATORID']+'\')" class="a_link">修改</a>';
          return returnText;
        }
      }
    ]
    ,url : '/lenovo/pcstoreorder/pagelist'
  });
});
...
function orderInfo(orderNo){
	window.location.href = "/lenovo/pcstoreorder/orderinfo?orderNo="+orderNo;
}
 ...
 * @return
 */
function operatorOrder(operatorId){
	window.location.href = "/lenovo/pcstoreorder/operatororder?operatorId="+operatorId;
}
