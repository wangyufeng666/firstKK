...
$().ready(function(){
  grid = $('#grid').grid({
    ...
    ,url : '/idlefish/crediturge/pagelist'
...
function withoutInfo(orderNo){
	var url = "/idlefish/crediturge/withoutinfo?orderNo="+orderNo;
	$.layer({
		type:2,
		title:'代扣详情',
		iframe:{src:url},
...
function urgeInfo(orderNo){
	var url = "/idlefish/crediturge/urgeinfo?orderNo="+orderNo;
	$.layer({
		type:2,
		title:'催缴详情',
		iframe:{src:url},
...
function executeUrge(orderNo, lastNoticeDate){
	...
	if(confirm(confirmText)){
		$.post('/idlefish/crediturge/executeurge', {orderNo:orderNo}, function(data){
			if(data == 'Y'){
				alert("催缴成功");
				doSearch();
			}else{
...
function downloadOrder(){
	...
	window.location.href = '/idlefish/tmallorder/orderexport?'+param;
