		...
		,cm:[
            ...
            ,{header:"操作", dataIndex:'SALEBILLNO', width:'110px',sortable:false,
                renderer:function(value, data, rowIndex, colIndex, metadata){
                	...
                	var html = "";
            		if(status == 2){
                		html += "<a class='a_link' href='javaScript:updateStatus(\"" + value + "\",\"3\")'>审核通过</a> | ";
                		html += "<a class='a_link' href='javaScript:updateStatus(\"" + value + "\",\"4\")'>驳回</a>";
                	}
                    return html;
                }
            }
 ...
 * 导出详情
 */
function explodeDetail(saleBillNo){
	if(saleBillNo){
		window.location.href = '/recycle/salebill/exportlistall?saleBillNo='+saleBillNo+'&classify='+classify;
	}else{
		alert('错误！稍后再试');
	}
}
...
$('#templateDownload').on('click',function(){
	window.location.href = '/template/xls/Finance_salebill_temp01.xls';
});
...
function dataImport(){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:'/recycle/salebill/ebsimportpage',
		area:['100%', '100%'],
		close:function(index){
			layer.close(index);
		}
