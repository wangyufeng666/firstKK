			...
			,{header: "操作", dataIndex: 'ORDERNO', width:'130px', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
					...
					if(status == '4' || status == '8'){
						returnText += ' | <a href="javascript:orderPayment(\''+value+'\')" class="a_link">付款</a>';
					}else if(status == '20'){
...
function orderInfo(orderNo){
	window.location.href = "/order/zhuanzhuan/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function orderPayment(orderNo){
	layer.open({
		...
		shadeClose:false,
		shade:0.8,
		content:"/order/zhuanzhuan/paypage?orderNo="+orderNo,
		area:['600px', '400px'],
...
function downloadOrder(){
	var param = '';
	param += 'partnerCode=10000053&orderType=72';
