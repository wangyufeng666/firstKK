...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm:[
			...
			,{header: "门店", dataIndex: 'STORENAME', width:'70px',sortable:false
				,renderer : function(value, data, rowIndex, colIndex, metadata){
					if(data['STORENO'])
			...
			,{header: "订单地址", dataIndex: 'DIZHI', width:'110px',sortable:false}
			,{header: "旧机序列号", dataIndex: 'OLDIMEI', width:'70px',sortable:false}
			,{header: "新机序列号", dataIndex: 'NEWIMEI', width:'70px',sortable:false}
			,{header: "订单来源", dataIndex: 'ORDERTYPENAME', width:'60px',sortable:false}
			,{header: "订单价格", dataIndex: 'ORDERPRICE', width:'60px',sortable:false}
			,{header: "成交价格", dataIndex: 'SETTLEPRICE', width:'60px',sortable:false}
			...
			,{header: "操作", dataIndex: '', width:'60px', sortable:false, 
				renderer : function(value, data, rowIndex, colIndex, metadata){
					...
					//待发券
					if(orderType == '29' || orderType == '30' || orderType == '31'){
						if(status == '8'){
...
function orderInfo(orderNo){
	window.location.href = "/order/sharp/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/asus/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}
