$().ready(function(){
	...
	$('#addForm').validate({
		rules:{
			questionName:{required:true, maxlength:50}//选型名称
			,questionDesc:{maxlength:100}//选项备注
		}
		,messages:{
			questionName:{required:"请输入规则类型"}//规则类型
			,questionDesc:{maxlength:"选项备注不能超过100字符"}//规则备注
		}
	});
});
...
function saveQuestion(thisAttrId){
	...
	if($("#addForm").valid()){
		...
		$.ajax({
			type:'POST'//请求方式
			,url:"/identify/rule/savequestion"  //请求路径
