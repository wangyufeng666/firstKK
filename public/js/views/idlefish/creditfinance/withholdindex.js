...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/idlefish/creditfinance/withholdpagelist'
...
function orderWithhold(ydmtradeNo, withPrice){
	if($('#quickWithhold').val() == 'Y'){
		$.post("/idlefish/creditfinance/withhold", {ydmtradeNo:ydmtradeNo}, function(data){
...
function withholdSplit(pkid){
	$.post("/idlefish/creditfinance/withholdsplit", {pkid:pkid}, function(data){
		if(data == 'Y'){
			doSearch();
		}else{
...
function downloadBackPayExport(){
	...
	window.location.href = '/idlefish/recyorder/backpayexport?'+param;
	return false;
}
...
function withholdInfo(pkid){
	window.location.href = "/idlefish/creditfinance/withholdinfo?pkid="+pkid+"&backUrl="+backUrl;
}
/**
 * 推送催缴
 */
function overdueCard(orderNo){
	if(confirm('超过七天扣款失败推送告知用户？')){
		$.post("/idlefish/creditfinance/pushcard", {orderNo:orderNo}, function(data){
			if(data == 'Y'){
				alert('推送成功');
				doSearch();
			}else{
