...
$("#partnerCode").change(function(){
    ...
    $.post("/device/divided/checktypecode", {typeCode:typeCode}, function(data) {
		if(data == 'Y'){
			alert('该代理已存在进行中的分成策略，如果需要开通新的分成策略，请先终止进行中的分成策略');
		}else{
...
function saveBrokerage(){
	...
	if($('#addForm').valid()){
		...
		$.post('/device/divided/savedivided', datas, function(data){
			if(data == 'Y'){
				goBack();
			}
		});
