...
//					    public static $SALE_PRODUCT_STATUS = Array('1'=>'待确认','2'=>'待出库','3'=>'待发货','4'=>'已发货',
//			                    '5'=>'退货收货中','6'=>'退货寄回中','7'=>'退货成功','8'=>'退货失败','99'=>'交易关闭',
//			                    '65'=>'订单取消','66'=>'交易成功','67'=>'交易失败','9'=>'退货确认中');
						...
						if(sendUserId && sendUserId != ''){
							if(status == '3'){//待发货的状态-》才能确认发货
							...
							}else if(status == '6' || status == '12'){//退货寄回中状态-》才能确认收货
								returnText +=' | <a href="javascript:updateOrderStatus(\''+orderNo+'\', 9)" class="a_link">同意退货</a>';
								if(status != '12'){
									returnText +=' | <a href="javascript:addExpressInfo(\''+orderNo+'\')" class="a_link">填写退货快递</a>';
									returnText +=' | <a href="javascript:updateOrderStatus(\''+orderNo+'\', 10)" class="a_link">退货驳回</a>';
								}
							}else if(status == '9'){//同意退货确认中-》打印退货单和退货成功和退货失败
								returnText +=' | <a href="javascript:printReturnOrder(\''+orderNo+'\')" class="a_link">打印退货单</a>';
								...
								returnText +=' | <a href="javascript:updateOrderStatus(\''+orderNo+'\', 11)" class="a_link">退货寄回</a>';
								returnText +=' | <a href="javascript:updateOrderStatus(\''+orderNo+'\', 12)" class="a_link">二次申请退货</a>';
							}else if(status == '7'){//退货成功-》换货
								returnText +=' | <a href="javascript:changeProduct(\''+orderNo+'\')" class="a_link">换货</a>';
							}
						}
 ...
 * @returns
 */
function printReturnOrder(orderNo){
	layer.open({
		type:2,
		title:'打印退货单',
		shadeClose:false,
		shade:0.8,
		content:'/sale/saleproduct/printreturnorder?orderNo='+orderNo,
		area:['90%','90%'],
		close:function(index){
			layer.close(index);
		}
 ...
 * @returns
 */
function changeProduct(orderNo){
	if(confirm('是否确认换货？')){
		$.post('/sale/saleproduct/changeproduct',{orderNo:orderNo},function(data){
			if(data == 'Y'){
				reload();
			}
		})	
...
function updateOrderStatus(orderNo, status, flag=null){
	...
	if(orderNo && status){
		...
		}else if(status == '9'){//同意退货
			layer.open({
				type:2,
				title:'同意退货',
				shadeClose:false,
				shade:0.8,
				content:'/sale/saleproduct/agreeorder?orderNo='+orderNo+'&thisStatus=9',
				area:['500px','350px'],
				close:function(index){
					layer.close(index);
		  	     }
			});	
		}else if(status == '10'){//驳回退货
			layer.open({
				type:2,
				title:'驳回退货',
				shadeClose:false,
				shade:0.8,
				content:'/sale/saleproduct/returnorder?orderNo='+orderNo+'&thisStatus=10',
				area:['500px','350px'],
				close:function(index){
					layer.close(index);
		  	     }
			});		
		}else if(status == '11'){//退货寄回
			layer.prompt({
				title: '请输入退货寄回的快递单号'}, 
				function(returnMailNo, index){
					layer.close(index);
					$.post('/sale/saleproduct/updateorderstatus',{orderNo:orderNo,thisStatus:status,returnMailNo:returnMailNo},function(data){
						if(data.code == '200'){
							reload();
						}
					})	
...
function doExport() {
	...
	params +='&storeType=' + $('#storeType').val();
	params +='&contacts=' + $('#contacts').val();
	params +='&orderStatus=' + $('#orderStatus').val();
	params +='&imei=' + $('#imei').val();
	window.location.href = '/sale/saleproduct/exportproduct?'+params;
	return false; //截取返回false就不会保存网页了
}
