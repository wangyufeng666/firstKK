...
function strategyInfo(strategyCode){
	window.location.href = "/recycle/redbag/strategyinfo?strategyCode="+strategyCode+"&backUri="+backUri;
}
...
function disableStrategy(strategyCode){
	if(confirm('是否将活动置为无效？')){
		$.post('/recycle/redbag/disablestrategy', {strategyCode:strategyCode}, function(data){
			if(data == 'Y'){
				doSearch();
			}else{
...
function editstrategy(strategyCode){
	$.layer({
		type:2,
		title:'修改策略信息',
		iframe:{src:'/recycle/redbag/editstrategyinfo?strategyCode='+strategyCode+'&backUri='+backUri},
		area:['750','550'],
		offset:['50px',''],
...
function enableStrategy(strategyCode){
	if(confirm('是否将活动置为有效？')){
		$.post('/recycle/redbag/enablestrategy', {strategyCode:strategyCode}, function(data){
			if(data == 'Y'){
				doSearch();
			}else{
...
function stopstrategy(strategyCode,strstartDate,currentdate){
    if(confirm('是否将活动置为终止？')){
    	if(strstartDate>currentdate){
    		$.post('/recycle/redbag/stopstrategytostatus', {strategyCode:strategyCode}, function(data){
    			if(data == 'Y'){
    				layer.alert("终止成功",1);
    				window.location.href ='/recycle/redbag/index';
    			}else{
    				alert('终止失败,请重试。');
    			}
    		});
    		...
    		$.layer({
    			type:2,
    			title:'终止佣金策略',
    			iframe:{src:'/recycle/redbag/stopstrategyinfo?strategyCode='+strategyCode+'&backUri='+backUri},
    			area:['550','300'],
    			offset:['50px',''],
    			close:function(index){
    				layer.close(index);
...
function useStrategy(strategyCode){
	if(confirm('启用后将无法修改操作，是否确认启用该佣金策略？')){
		$.post('/recycle/redbag/usestrategy', {strategyCode:strategyCode}, function(data){
			if(data == 'Y'){
				doSearch();
			}else{
...
function addBrokerageStrategy(){
	window.location.href = '/recycle/redbag/addbrokeragestrategy?backUri='+backUri;
}
