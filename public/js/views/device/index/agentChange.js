...
//代理商动态改变
$('#qu').change(function(){
	$("#typeCode").html("<option flag='' value=''>请选择代理商</option>");
	var thisVal = $(this).val();
	if(thisVal != ''){
		$('#quName').val($(this).find("option:selected").text());
		$.ajax({type:'GET',dataType:'jsonp',cache:false,jsonp:'jsonp_callback',data:{areaid:thisVal},url:'/device/agents/getagent',
			success:function(data){
				var optionHtml = '',id = '',name = '';
				for(i in data){
					id = data[i]['ID'];
