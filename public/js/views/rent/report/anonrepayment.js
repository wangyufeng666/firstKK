...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url : '/rent/report/anonrepaymentpage'
...
$('#exportlist').on('click',function(){
	var days = $('#days').val();
	var param = '?days='+days;
	window.location.href = '/rent/report/anonrepaymentexport'+param;
});

function infos(start_date){
	if(start_date){
		window.location.href='/rent/report/anonrepaymentinfo?start_date='+start_date;
	}else{
		alert('错误，稍后再试');
	}
}