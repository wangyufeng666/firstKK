...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,url:'/idlefish/activity/userprizepagelist'
...
function grantAndStop(userid,types,prizeid,orderno){
	...
	if(hint){
		if(confirm(hint)){
			if(userid && prizeid){
				$.post('/idlefish/activity/grantandstop',{userid:userid,types:types,prizeid:prizeid,orderno:orderno,go:'1'},function(data){
					if(data == 'Y'){
						alert('操作成功');
					}else{
						alert('操作失败');
