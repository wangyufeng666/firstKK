...
$().ready(function(){
	grid = $('#grid').grid({
		...
		,cm : [
           ...
           ,{header: "操作", dataIndex: '', width:'12%', sortable:false,
				renderer : function(value, data, rowIndex, colIndex, metadata){
				  	var returnText ='<a class="a_link" href="javascript:void(0);" onclick="iceboxInfo(\''+data['SPID']+'\')">详情</a>';
        ...
        ,url : '/product/icebox/pagelist'
        ,baseParams:{merType:'Q', merName:$('#merName').val(),pinpai:$('#pinpai').val()}
	});
});
 ...
 * @return
 */
function iceboxInfo(spId){
	var start = $('#start').val();
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/icebox/iceboxinfo?spId="+spId+"&merName="+merName+"&pinpai="+pinpai;
}
...
function addIcebox(){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/icebox/addicebox?pinpai="+pinpai+"&merName="+merName;
}
...
function edit(spId){
	var pinpai = $('#pinpai').val();
	var merName = $('#merName').val();
	window.location.href = "/product/icebox/editicebox?spId="+spId+"&pinpai="+pinpai+"&merName="+merName;
}
...
function deleteThis(spId){
	if(confirm('是否确认删除选中的商品？')){
		$.post('/product/icebox/deleteicebox', {spId:spId}, function(data){
			if(data == 'Y'){
				doSearch();
			}
		});
