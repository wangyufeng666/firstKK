...
$().ready(function(){
  grid = $('#grid').grid({
    ...
    ,cm : [
      ...
      ,{header: "订单渠道", dataIndex: 'PARTNERNAME', width:'80px',sortable:false}
      ,{header: "订单来源", dataIndex: 'SOURCENAME', width:'100px',sortable:false}
      ,{header: "订单价格", dataIndex: 'ORDERPRICE', width:'70px',sortable:false
    	,renderer : function(value, data, rowIndex, colIndex, metadata){
    	  if(data['SETTLEPRICE']){
    		  return data['ORDERPRICE']+'<font color="red">('+data['SETTLEPRICE']+')</font>';
    	  }else{
...
//      ,{header: "成交价格", dataIndex: 'SETTLEPRICE', width:'70px',sortable:false}
      ...
      ,{header: "操作", dataIndex: '', width:'80px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
          ...
          //1:待审核； 2:待上门； 6:待发货；7:待收货；3:待验货；4:待付款；8:待发券；20:发券中；5:待入库；66:交易成功；99：终止
    	  if((status == '4' || status == '8') && techSupport == '0'){
			  returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\')" class="a_link">支付</a>';
          }else if(status == '20' && techSupport == '0'){
        	  returnText += ' | <a href="javascript:void(0);" onclick="sendCoupon(\''+orderNo+'\')" class="a_link">确认支付</a>';
          }
          return returnText;
        }
      }
    ]
    ,url : '/order/mallorder/pagelist'
...
function orderInfo(orderNo){
	window.location.href = "/order/mallorder/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/mallorder/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}
