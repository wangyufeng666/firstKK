    ...
    ,cm : [
      ...
      ,{header: "操作", dataIndex: '', width:'130px', sortable:false, 
        renderer : function(value, data, rowIndex, colIndex, metadata){
    	  ...
    	  //待发券
    	  if(orderType == '300' || orderType == '301'){
    	      if(status == '8'){
...
function orderInfo(orderNo){
	window.location.href = "/order/mogujie/orderinfo?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function sendCoupon(orderNo){
	window.location.href = "/order/mogujie/sendcouponpage?orderNo="+orderNo+"&backUrl="+backUrl;
}
...
function doExport(){
    ...
    var param = 'merName='+merName+'&orderNo='+orderNo+'&orderType='+orderType+'&tradeType='+tradeType+'&contactWay='+contactWay+'&orderStatus='+orderStatus;
    	param += '&startDate='+startDate+'&endDate='+endDate+'&category='+category+'&merType='+merType+'&address='+address;
	window.location.href = "/order/mogujie/exportexecl?"+encodeURI(param);
}